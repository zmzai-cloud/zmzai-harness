...
async function git(args: string[]): Promise<string> {
  const { stdout } = await run("git", args, { cwd: activeWorkspaceRoot(), maxBuffer: 10 * 1024 * 1024 });
  return stdout.trim();
}
...
export async function GET() {
  try {
    const log = await git(["log", "--format=%H%x00%ct%x00%s", `-n`, `30`]).catch(() => "");
    const points = log
      .split("\n")
      .filter(Boolean)
      .map((line) => {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const out = await git(["commit", "-m", `${MARKER} ${label}`, "--allow-empty-message", "--no-verify"]);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    await git(["clean", "-fd"]).catch(() => undefined);
