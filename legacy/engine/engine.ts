...
/** 默认模型引用：来自 .env 的 OPENAI_MODEL（relay 登录态下即 deepseek-chat）。 */
function defaultModelRef(): ModelRef {
  return { providerId: "openai", modelId: process.env.OPENAI_MODEL ?? "gpt-4o" };
}
...
export class EngineRuntime {
  ...
  constructor(opts: {
    ...
    /** 动态请求头（如 relay 登录态 cookie），每次 LLM 调用前求值。 */
    ...
    const modelProvider = createOpenAiModelProvider({ headers: opts.modelHeaders });
  ...
  async createSession(input: { agent?: string; model?: ModelRef }): Promise<SessionInfo> {
    return this.fw.createSession({
      ...
      workspaceId: "local",
      agent: input.agent,
      model: input.model ?? defaultModelRef(),
    });
  }
  ...
  listAgents(): AgentInfo[] {
    // 未显式配置模型的 agent 补上默认模型，UI 不显示空值
    return this.fw.registry.list({ includeHidden: false }).map((a) => ({ ...a, model: a.model ?? defaultModelRef() }));
  }
  ...
  // ---------------------------------------------------------------------------
  // 交互式终端（UI 直连）：默认 cwd 绑工作区根，输出用 read(id, cursor) 增量拉取
  // ---------------------------------------------------------------------------
  startTerminal(input: { name?: string; command: string; cols?: number; rows?: number }) {
  ...
  readTerminal(id: string, sinceBytes?: number) {
    const r = this.#terminalManager.read(id, sinceBytes);
    return r ? { ...r, session: { ...r.session } } : null;
  }

  writeTerminal(id: string, text: string): boolean {
    return this.#terminalManager.write(id, text);
  }
  ...
  killTerminal(id: string): boolean {
    return this.#terminalManager.kill(id);
  }
