        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
          ...
          {sessions.map((s) => (
            <button
              ...
              className={`mb-0.5 block w-full rounded-sm px-3 py-2 text-left transition-colors ${
                activeId === s.id ? "bg-bg shadow-sm ring-1 ring-line" : "hover:bg-surface-2"
              }`}
            >
              <div className="flex items-baseline justify-between gap-2">
                <span className={`truncate text-[0.8125rem] font-medium ${activeId === s.id ? "text-ink" : "text-ink-2"}`}>
                  {s.title || "未命名会话"}
                </span>
