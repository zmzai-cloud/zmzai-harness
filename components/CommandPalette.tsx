...
/**
 * 命令面板（P2-12）：⌘K 命令 / ⌘P 文件快开（IDE 感标配）。
 * mode "commands" 渲染传入命令列表；mode "files" 走 /api/fs/search 递归文件名搜索，
 * 选中后 onOpenFile 打开产物侧文件 Tab。
 */
export default function CommandPalette({
  ...
  const items = useMemo<{ key: string; label: string; hint?: string; run: () => void }[]>(() => {
    if (mode === "commands") {
  ...
  }, [mode, commands, fileHits, query, onOpenFile]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-ink/25 pt-[14vh]" onClick={onClose}>
      ...
      >
        <input
          ...
          placeholder={mode === "files" ? "搜索文件名…（⏎ 打开）" : "输入命令…"}
          ...
          {items.map((it, i) => (
            <button
              ...
              className={cn(
                ...
                i === index ? "bg-bg" : "hover:bg-bg",
              )}
            >
              <span className="truncate font-mono text-xs text-ink">{it.label}</span>
        ...
        <div className="flex h-7 items-center gap-3 border-t border-line px-3 text-[0.625rem] text-ink-3">
          ...
          <span>{mode === "files" ? "⌘P 文件" : "⌘K 命令"}</span>
        </div>
      </div>
    </div>
  );
